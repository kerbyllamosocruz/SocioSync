// Chrome Extension Polyfill for Tampermonkey APIs
const storageCache = {};
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  ...
  for (const [key, { oldValue, newValue }] of Object.entries(changes)) {
    ...
    valueChangeListeners.forEach((listenerObj) => {
      if (listenerObj.key === key) {
        try {
          listenerObj.callback(key, oldValue, newValue, true);
        } catch (e) {
          console.error('[Extension Polyfill] Listener error:', e);
        }
      }
    });
  }
...
function GM_getValue(key, defaultValue) {
  return storageCache[key] !== undefined ? storageCache[key] : defaultValue;
}
...
function GM_xmlhttpRequest(details) {
  chrome.runtime.sendMessage(
    ...
    (resp) => {
      if (!resp) {
        if (details.onerror) details.onerror(new Error('No response from background worker'));
        return;
      }
      if (resp.success) {
        if (details.onload) details.onload(resp.response);
      } else {
...
function GM_openInTab(url, options) {
  chrome.runtime.sendMessage({ type: 'GM_openInTab', url, options });
  return { close: () => {} };
}

// Populate storageCache then run main script
